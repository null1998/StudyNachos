 ...
 * object.
 *TCB模拟了必要的底层细节，用于Nachos的创建、上下文切换和销毁操作。每个TCB控制了一个底层JVM线程
 * <p>
 ...
 * 不要在任何地方使用synchronized同步锁，违背了初衷且很容易造成nachos死锁
 */
public final class TCB {
	 ...
	 * 如果不是第一个TCB，确保当前线程对应当前TCB，非Nachos线程使用start时失败
	 */
	if (!isFirstTCB)
	...
	if (!isFirstTCB) {
	     ...
	     * 尽管线程尚未start，仍需要通过yield将它阻塞，我们临时关闭当前TCB，开启新线程，等待其通过threadroot方法唤醒当前TCB
	     * 当新的TCB将当前TCB唤醒，就可以安全地将上下文切换至新的TCB
	     */
